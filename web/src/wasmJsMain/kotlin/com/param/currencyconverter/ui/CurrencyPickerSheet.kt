...
import com.param.currencyconverter.resources.cd_clear_search
import com.param.currencyconverter.resources.cd_selected
import com.param.currencyconverter.resources.picker_all
import com.param.currencyconverter.resources.picker_no_match
import com.param.currencyconverter.resources.picker_recent
import com.param.currencyconverter.resources.picker_search_placeholder
...
/**
 * Klarname zu einem Code, über den Browser statt über eine eigene Tabelle.
 *
 * Auf Android macht das `java.util.Currency`; im Browser ist
 * `Intl.DisplayNames` das Gegenstück und liefert den Namen in der
 * **Browsersprache** — dieselbe Idee: Übersetzungen, die die Plattform
 * ohnehin mitbringt, statt 166 Namen pro Sprache zu pflegen.
 *
 * `fallback: "none"` sorgt dafür, dass ein unbekannter Code `undefined`
 * ergibt statt einfach der Code selbst; ein ungültiger (falsche Länge)
 * wirft — beides landet über den try/catch als leerer String und hier
 * als `null`.
 */
internal fun currencyDisplayName(code: String): String? =
    jsCurrencyDisplayName(code).takeIf { it.isNotEmpty() && it != code }
...
private fun jsCurrencyDisplayName(code: String): String = js(
    ...
    (() => {
        try {
            return new Intl.DisplayNames(undefined, { type: "currency", fallback: "none" }).of(code) ?? "";
        } catch (e) {
            return "";
        }
    })()
...
internal fun CurrencyPickerSheet(
    ...
    ModalBottomSheet(
        ...
        OutlinedTextField(
            ...
            singleLine = true,
            placeholder = { Text(stringResource(Res.string.picker_search_placeholder)) },
        ...
        if (filtered.isEmpty()) {
            ...
            // auffällt.
            Text(
                text = stringResource(Res.string.picker_no_match, query),
        ...
        LazyColumn(modifier = Modifier.padding(top = 8.dp)) {
            if (showRecents) {
                item { SectionLabel(stringResource(Res.string.picker_recent)) }
                ...
                item { SectionLabel(stringResource(Res.string.picker_all)) }
            }
...
private fun List<CurrencyOption>.filterAndRank(query: String): List<CurrencyOption> {
    ...
    return mapNotNull { option ->
        val code = option.code.lowercase()
        val name = option.name?.lowercase().orEmpty()
        val rank = when {
            code.startsWith(q) -> 0
            name.startsWith(q) -> 1
